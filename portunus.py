...
def parse_args():
    ...
    auth = install.add_mutually_exclusive_group(
    ...
    install.add_argument('-k', '--key', type=str, nargs=1,
                         help='randomization key for which port location of ssh will be open', default=False)
    install.add_argument('-r', '--range', type=int, nargs=2, help='range of ports the ssh shell wil be hosted on', default=*(49152, 65535))
    install.add_argument('-t', '--time', type=int, nargs=1,
                         help='time (in minutes) between when the shell will change port locations', default=3600)
    install.add_argument('-n', '--name', type=str, nargs=1,
